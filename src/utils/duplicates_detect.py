...
def detect_duplicates_bronze(df:DataFrame, table_name:str)-> DataFrame:
    ...
    try:
        ...
        df=df.withColumn(
            "_dq_is_duplicate",
            when(
                col("existing._bronze_ingestion_batch").isNotNull(),True
            ).otherwise(False)
        )

        df=df.withColumn(
            "_dq_duplicate_of_batch",
            col("existing._bronze_ingestion_batch")
        )

        df=df.drop("existing._bronze_ingestion_batch")
